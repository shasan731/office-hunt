...
export class ResultsScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const breakdown = [
      ...
      ['Coding', `${state.bugsFixed} fixed / ${state.bugsMissed} missed`], ['Bug Bash', state.testerDefeated ? `Won ${state.fightRoundsWon}-${state.fightRoundsLost} • ${state.fightHealthRemaining}% HP` : 'QA prevailed'], ['Break levels', `Lunch ${state.lunchCompleted ? 'passed' : 'timed out'} • Tea ${state.teaQuestCompleted ? 'brewed' : 'cold'}`],
